...
namespace tools
{
template <typename T, bool PopWhenFull = false>//新数据直接丢弃，队列维持原样
...
public:
  ...
  void push(const T & value)
  {
    ...
    queue_.push(value);//把元素放入底层
    not_empty_condition_.notify_all();//条件变量,唤醒正在等待 take () 的消费者线程
  }

  void pop(T & value)
  {
    std::unique_lock<std::mutex> lock(mutex_);//对互斥量 `mutex_` 加锁，保护临界区代码，防止多线程同时读写共享资源
...
private:
  std::queue<T> queue_;
  size_t max_size_;
  mutable std::mutex mutex_;//允许const 成员函数内部修改该成员变量
